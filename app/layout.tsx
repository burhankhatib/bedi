      ...
      <body className="font-sans antialiased">
        ...
        <Script
          ...
          dangerouslySetInnerHTML={{
            __html: `(function(){var p=typeof location!=='undefined'&&location.pathname||'';if(!('serviceWorker' in navigator))return;if(p==='/driver'||p.indexOf('/driver/')===0||p==='/dashboard'||p.indexOf('/dashboard/')===0||p==='/admin'||p.indexOf('/admin/')===0){return;}var m=p.match(/^\\/t\\/([^/]+)\\/?/);if(m){var slug=m[1];if(p.indexOf('/t/'+slug+'/orders')===0){return;}}var ok=p==='/'||p==='/search'||p==='/my-orders'||p.indexOf('/order')===0||p.indexOf('/resolve')===0||p.indexOf('/join')===0||m;if(ok){navigator.serviceWorker.register('/customer-sw.js',{scope:'/'}).catch(function(){});}})();`,
          }}
        />
