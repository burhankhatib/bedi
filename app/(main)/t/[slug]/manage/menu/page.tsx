...
export default async function ManageMenuPage({
  ...
  const { categories, products } = await client.fetch<{
    categories: Array<{ _id: string; title_en: string; title_ar: string; slug: string; sortOrder?: number; productSortMode?: string; parentCategoryRef?: string }>
    products: Array<{
      _id: string
      title_en: string
      title_ar: string
    ...
    `{
      "categories": *[_type == "category" && site._ref == $siteId] | order(sortOrder asc) { _id, title_en, title_ar, "slug": slug.current, sortOrder, productSortMode, "parentCategoryRef": parentCategory._ref },
