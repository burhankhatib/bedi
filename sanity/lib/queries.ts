...
export const MENU_QUERY = defineQuery(`{
  "categories": *[_type == "category"] | order(sortOrder asc) {
    ...
    "products": *[_type == "product" && references(^._id)] | order(sortOrder asc) {
      ...
      title_en,
      title_ar,
      "description_en": coalesce(description_en, catalogRef->description_en, masterCatalogRef->description_en),
      "description_ar": coalesce(description_ar, catalogRef->description_ar, masterCatalogRef->description_ar),
      ingredients_en,
      ingredients_ar,
      price,
      saleUnit,
      ...
      specialPriceExpires,
      currency,
      "image": coalesce(image, catalogRef->images[0], masterCatalogRef->image),
      additionalImages,
      isPopular,
      isAvailable,
      availableAgainAt,
