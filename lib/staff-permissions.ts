...
export type StaffRole = 'waiter' | 'cashier' | 'manager' | 'custom'
...
export function getStaffPermissions(member: StaffMemberForAuth): StaffPermission[] {
  ...
  if (override.length > 0) {
    return [...new Set(override.filter((p): p is StaffPermission => STAFF_PERMISSIONS.includes(p as StaffPermission)))]
  }
  return rolePerms
}
