...
type EntityType = 'tenant' | 'driver' | 'customer'

const pathSegment = (t: EntityType) => (t === 'tenant' ? 'tenants' : `${t}s`)
...
export function BlockToggle({
  ...
  const handleToggle = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/admin/${pathSegment(type)}/${id}/block`, {
        method: 'PATCH',
  ...
  return (
    <button
      ...
      aria-label={checked ? 'Blocked — click to unblock' : 'Click to block'}
      disabled={loading}
      ...
      className={`
        ...
        ${checked ? 'border-rose-500/60 bg-rose-500/30' : 'border-slate-600 bg-slate-700/60'}
      `}
      title={checked ? 'Blocked — click to unblock' : 'Click to block (Super Admin only)'}
    >
      <span
        className={`
